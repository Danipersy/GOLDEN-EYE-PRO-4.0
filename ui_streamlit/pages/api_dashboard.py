        ...
        with st.container(border=True):
            ...
            with cols[1]:
                if isinstance(stat['limit'], int):
                    ...
                    st.progress(min(progress, 1.0), text=f"{stat['today']}/{stat['limit']}")
                else:
                    st.progress(0.1, text=f"{stat['today']} chiamate")
            with cols[2]:
                if isinstance(stat['limit'], int):
                    color = "#00ff88" if stat['percent'] < 50 else "#f0b90b" if stat['percent'] < 80 else "#ff3344"
                    st.markdown(f"<span style='color:{color};'>{stat['percent']:.0f}%</span>", unsafe_allow_html=True)
                else:
                    st.markdown("∞")
            with cols[3]:
